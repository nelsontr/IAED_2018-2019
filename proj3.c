...
int sala_ocupada(int sala,int data2,int hora,int dura){
    ...
    for (i=1;i<=count_room[sala];i++){
        ...
        if ((atoi(even.date) == data2) && ((hora>inicio_eve && hora<final_eve) 
        || (inicio_eve<final && final<final_eve) || (hora>inicio_eve && final<final_eve) 
        || (hora<inicio_eve && final>final_eve) || (hora==inicio_eve))){
            return 1;
        }
    }
    return 0;
...
int main(){
    ...
    char comand,str[MAX_STR],aux[MAX_STR];
    ...
    while (1){
        ...
        switch(comand){
            ...
            case 'i': /*Troca o inicio de um evento*/
                strcpy(aux,eve[room][num_eve].begin);
                strcpy(eve[room][num_eve].begin,list[1]);
                printf("%s",eve[room][num_eve].begin);
                if (sala_ocupada(room,atoi(eve[room][num_eve].date),atoi(list[1]),eve[room][num_eve].min)){
                    printf("Impossivel agendar evento %s. Sala%d ocupada.\n",list[0],room);
                    strcpy(eve[room][num_eve].begin,aux);
                }
                else
                    strcpy(eve[room][num_eve].begin,list[1]);             
                break;
